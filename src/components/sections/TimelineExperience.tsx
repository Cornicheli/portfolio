...
export default function TimelineExperience({ experiences }: Props) {
  ...
  const hasImages = (id: string) =>
    ImagenesJobs.some((img) => img.id === id);
        ...
        {experiences.map((exp, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Right indicator */}
              <div className={`self-center relative z-[1] transition-all duration-[250ms] ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                {clickable ? (
                  <div
                  ...
                  <span style={{ color: "var(--ink-5)", fontSize: 20 }}>↗</span>
                )}
              </div>
            </article>
          );
